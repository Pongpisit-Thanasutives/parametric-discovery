...
def first_sig(n, tol=int(1e2)):
    return float(('{'+f":.{len(str(n))+tol}e"+'}').format(n).split('e')[0])
...
def RPCA(U, lam_2 = 1e-3, verbose=False):
    Y1         = U
    ...
    while iter< maxIter and err > tol:
        ...
        if (iter % iter_print) == 0 or iter == 1 or iter > maxIter or err < tol:
            if verbose: print('iteration:{0}, err:{1}, nc_norm:{2} eta1:{3}'.format(iter, err, nc_norm, eta1))
...
def TrainSTRidgeFixed(R, Ut, lam, d_tol, maxit = 25, STR_iters = 10, l0_penalty = None, normalize = 2, split = 0.8, print_best_tol = False, print_eq=True, threshold=None, recursive=False, norm_type=2, use_sindy_model=False, feature_names=None, max_feature=None, cond_num_sig=False, seed=0):
    """
    This function trains a predictor using STRidge.
